...
@Injectable()
export class TransformResponseInterceptor<T extends DataType>
  implements NestInterceptor<T, ISuccessResponse<T> | T>
{
  ...
  intercept(
    ...
    next: CallHandler<T>,
  ): Observable<ISuccessResponse<T>> | Observable<T> {
    const bypassTransformResponse = this.reflector
      ? this.reflector.get<boolean>(
          'bypass-transform-response',
          context.getHandler(),
        )
      : false;
